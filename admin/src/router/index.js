...
router.beforeEach((to, from, next) => {
  if (!to.meta.authRequired) {
    next();
  } else if (store.getters["authenticated"] && store.getters["user"].role == 'admin') {   
    next();
  } else {
    next({
      path: '/auth-pages/login',
      ...
      children: [
        {
          path: 'source/:id',
          name: 'source-list',
          component: () => import('@/pages/sources/Index'),
          meta: {
            authRequired: true
          }
        },
        {
          path: 'edit/:id',
          name: 'source-edit',
          component: () => import('@/pages/sources/Edit'),
          meta: {
            authRequired: true
          }
        },
        {
          path: 'create/:id',
          name: 'source-create',
          component: () => import('@/pages/sources/Create'),
          meta: {
            authRequired: true
          }
        },
